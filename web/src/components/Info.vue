<template>
  ...
  <div class="base-container flex justify-center">
    <div class="w-full px-4 sm:px-6 md:px-8 lg:px-0 lg:w-2/3 xl:w-1/2 py-8 sm:py-10 md:py-12">
      ...
      <div class="space-y-2">
        <div v-for="(project, index) in resume.selectedWorks" :key="index" class="py-4">
          ...
          <div class="flex flex-col gap-1 sm:hidden">
            <span class="text-sm uppercase tracking-wide text-gray-500 font-semibold">{{ project.date }}</span>
            ...
            <span class="text-sm text-gray-500 font-medium">{{ project.released }}</span>
            <p class="text-sm text-gray-600 leading-relaxed mt-1">{{ project.description }}</p>
          </div>
          <!-- Desktop -->
          <div class="hidden sm:block">
            <div class="flex relative items-center gap-4">
              <span class="text-sm uppercase tracking-wide text-gray-500 font-semibold whitespace-nowrap w-24">{{ project.date }}</span>
              ...
              <span class="text-sm uppercase tracking-wide text-gray-500 font-medium whitespace-nowrap">{{ project.released }}</span>
              <a v-if="project.link" :href="project.link" target="_blank" rel="noopener noreferrer" class="release-button" aria-label="Listen to track, opens in new window">LISTEN</a>
            </div>
            <p class="text-sm text-gray-600 leading-relaxed mt-2 ml-34">{{ project.description }}</p>
          </div>
        </div>
      </div>

      <h3 class="project-category mt-20 sm:mt-24">COLLABORATIONS</h3>
      <div class="space-y-2">
        <div v-for="(project, index) in resume.collaborations" :key="index" class="py-4">
          ...
          <div class="flex flex-col gap-1 sm:hidden">
            ...
            <span class="text-sm text-gray-500 font-medium">{{ project.type }} @ {{ project.where }}</span>
          </div>
          <!-- Desktop -->
          <div class="hidden sm:flex relative items-center gap-4">
            ...
            <div class="absolute top-0 bottom-0 bg-black" style="width: 1px; left: 100px;"></div>
            <span class="text-base text-gray-900 font-medium tracking-wide flex-1 pl-6">{{ project.title }}</span>
            <span class="text-sm text-gray-500 font-medium whitespace-nowrap">{{ project.type }} @ {{ project.where }}</span>
            <a v-if="project.link" :href="project.link" target="_blank" rel="noopener noreferrer" class="release-button" aria-label="View project, opens in new window">VIEW</a>
          </div>
        </div>
      </div>
    ...
    <section v-if="releases.releases.length" class="mt-20 sm:mt-24">
      ...
      <div class="space-y-2">
        <div v-for="(release, index) in releases.releases" :key="index" class="py-4">
          <!-- Mobile -->
          <div class="flex flex-col gap-1 sm:hidden">
            <span class="text-sm uppercase tracking-wide text-gray-500 font-semibold">{{ release.releaseDate.slice(0, 4) }}</span>
            <div class="flex items-center justify-between gap-4">
            ...
            <span class="text-sm text-gray-500 font-medium">{{ release.label }}</span>
            <p class="text-sm text-gray-600 leading-relaxed mt-1">{{ release.description }}</p>
          </div>
          <!-- Desktop -->
          <div class="hidden sm:block">
            <div class="flex relative items-center gap-4">
              <span class="text-sm uppercase tracking-wide text-gray-500 font-semibold whitespace-nowrap w-24">{{ release.releaseDate.slice(0, 4) }}</span>
              <div class="absolute top-0 bottom-0 bg-black" style="width: 1px; left: 100px;"></div>
              <span class="text-base text-gray-900 font-medium tracking-wide flex-1 pl-6">{{ release.artist }} - {{ release.title }}</span>
              <span class="text-sm uppercase tracking-wide text-gray-500 font-medium whitespace-nowrap">{{ release.label }}</span>
              <a v-if="release.bandcamp_link" :href="release.bandcamp_link" target="_blank" rel="noopener noreferrer" class="release-button" aria-label="Listen to release, opens in new window">LISTEN</a>
            </div>
            <p class="text-sm text-gray-600 leading-relaxed mt-2 ml-34">{{ release.description }}</p>
          </div>
        </div>
      </div>
    </section>
...
function extractYear(title) {
  // Extract year or year range from parentheses like (Label, 2019) or (2021-2025)
  const match = title.match(/\(.*?(\d{4}(?:-\d{4})?)\)/);
  return match ? match[1] : null;
}

function extractTitle(title) {
  // Remove content in parentheses to get clean title
